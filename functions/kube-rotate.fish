function __kube_rotate_print_help -d "Prints help text"
  echo "usage: kube-rotate -n|--node=<k8s_node> -p|--profile=<aws_profile> [-c|--context=<k8s_context] [-r|--region=<aws_region>]"
end

function kube-rotate -d "Drains a Kubernetes node & terminates the instance"
  ...
  argparse -n "kube-rotate" $options -- $argv
  ...
  kubectl --context=$_flag_c delete node $k8s_node

  if test $status -eq 0
    set -l instance_id (kube-instance -n$k8s_node -p$_flag_p -r$_flag_r)

    aws --profile $_flag_p ec2 terminate-instances \
